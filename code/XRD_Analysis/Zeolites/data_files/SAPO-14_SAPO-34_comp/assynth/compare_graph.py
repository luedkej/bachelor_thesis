...
def plot_xy_files():
    ...
    for i, file_name in enumerate(xy_files):
        ...
        axs[i].axvline(
            x=11, color="red", linestyle="--", linewidth=1
        )  # Add vertical line
        axs[i].axvline(
            x=13, color="blue", linestyle="--", linewidth=1
        )  # Add vertical line
        axs[i].axvline(
            x=16, color="black", linestyle="--", linewidth=1
        )  # Add vertical line
        axs[i].axvline(
            x=18, color="black", linestyle="--", linewidth=1
        )  # Add vertical line
        axs[i].axvline(
            x=22.5, color="green", linestyle="--", linewidth=1
        )  # Add vertical line
        axs[i].axvline(
            x=31, color="violet", linestyle="--", linewidth=1
        )  # Add vertical line
    ...
    plt.xlim(5, 40)
    plt.tight_layout()
    plt.savefig("SAPO-14_SAPO-34_assynth_combined.png")
    plt.show()
